...
export function dumpIdMap(idMap): void {
    _.forIn(idMap, (obj, id) => {
        ...
        _.forIn(obj, (v, k) => {
            ...
            if (Array.isArray(v) && v.length > 0 && _.has(obj, `${k}[0].$oid`)) {
                let relationship: Array<string> = [];
                let isPushedT = false;
                for (let i = 0; i < v.length; ++i) {
                    const kidId = <string>_.get(obj, `${k}[${i}].$oid`);
                    if (!(kidId in idMap))
                        zombies[kidId] = true;
                    else {
...
export function bindIdMap(idMap): void {
    _.forIn(idMap, (obj, id) => {
        _.forIn(obj, (v, k) => {
            const T = obj["T"];
            /*
...
export function findActs(idMap, subjects: Array<string>, grades: Array<number>): Array<object> {
...
export function numActs(idMap, status?) {
